 ...
 * tools use (see {@code FileReadTool}, {@code FileEditTool}). A tool is not
 * itself a {@code ServiceLoader}-discovered type; it's only reachable by
 * the framework once it's bundled into a {@link ExampleSkill}'s {@code
 * tools()} list (see that class), which *is* discovered via {@code
 * ServiceLoader}.
 */
