...
// main repo) so the compiler can see the Agent/EdgeCondition/Skill/
// ToolBundle API packages. No Spring Boot plugin, no fat-jar shading
// needed — this produces a plain jar that the framework loads at runtime
// via its own classpath, not yours.
plugins {
    id("java")
}
...
dependencies {
    ...
    // ToolBundle.tools() bundles a Spring AI ToolCallback — spring-ai-commons +
    // spring-ai-model provide the @Tool annotation and ToolCallback/
    // ToolCallbacks types used by ExampleTool/ExampleToolBundle.
